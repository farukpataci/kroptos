...
/**
 * Adapter allowing İkas to be consumed through the MarketplaceConnector
 * interface whenever a caller queries MarketplaceConnectorFactory.
 */
export class IkasMarketplaceAdapter extends MarketplaceConnector {
  private readonly delegate: IkasConnector;
  ...
  constructor(
    ...
    this.delegate = new IkasConnector(credentials, ecommerceHttpClient, ecommerceRateLimiter, settings);
  }
  ...
  override async getOrders(): Promise<MarketplaceOrder[]> {
    ...
    return orders.map((o) => ({
      ...
      shippingAddress: o.shippingAddress
        ? [o.shippingAddress.address1, o.shippingAddress.province, o.shippingAddress.city, o.shippingAddress.country]
            .filter(Boolean)
            .join(' ')
        : undefined,
      ...
      shippingCountryCode: o.shippingAddress?.countryCode || o.shippingAddress?.country,
      items: o.items.map((i) => ({
        sku: i.sku || i.id,
        name: i.title,
        quantity: i.quantity,
