/**
 * Fatture in Cloud (TeamSystem) PRODUCTION Client
 *
 * CRITICAL RULE (§1, §6, §10):
 * In MOCK_READY phase, no real network requests are permitted.
 * Calling any method on ProductionClient throws IntegrationNotVerifiedError.
 */

import { IntegrationNotVerifiedError } from '../core/AccountingErrors';
import { FIC_PROVIDER_NAME } from './fic.types';

export class FicProductionClient {
  private guard(): never {
    throw new IntegrationNotVerifiedError(FIC_PROVIDER_NAME, 'PRODUCTION');
  }
