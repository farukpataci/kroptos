...
export class BizimhesapProductionClient implements IBizimhesapClient {
  ...
  async testConnection(): Promise<AccountingTestConnectionResult> {
    throw new IntegrationNotVerifiedError(
      'BIZIMHESAP',
      'BizimHesap PRODUCTION ortamı henüz doğrulanmamıştır. Yalnızca MOCK ortamı desteklenmektedir.',
    );
  }
  ...
  async createInvoice(_request: AccountingInvoiceRequest): Promise<AccountingInvoiceResult> {
    throw new IntegrationNotVerifiedError(
      'BIZIMHESAP',
      'BizimHesap PRODUCTION ortamı henüz doğrulanmamıştır. Gerçek ağ isteği yapılamaz.',
    );
  }
