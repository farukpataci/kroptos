    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-kp-border pb-4">
        <div>
          ...
          <p className="mt-1 text-[13px] text-kp-text-tertiary">
            Monitor stock levels, warehouse inventory, reservations, replenishment, stock movements and alerts.
          </p>
        </div>
      ...
      <div className="flex border-b border-kp-border overflow-x-auto scrollbar-hide mb-4">
        {[
          ...
          { id: 'ledger', label: 'Stock Ledger', icon: CubeIcon },
          { id: 'map', label: 'Warehouse Map', icon: MapIcon },
          { id: 'movements', label: 'Movements Log', icon: ClockIcon },
          { id: 'lowstock', label: 'Replenishment', icon: ExclamationTriangleIcon },
          { id: 'counting', label: 'Counting / Audits', icon: AdjustmentsHorizontalIcon },
          { id: 'reservations', label: 'Reservations', icon: TagIcon },
          { id: 'integrations', label: 'Integrations', icon: InboxArrowDownIcon },
          { id: 'ai', label: 'AI Insights', icon: CpuChipIcon },
          { id: 'alerts', label: 'Alert Center', icon: BellIcon }
        ].map((tab) => {
          const Icon = tab.icon;
          ...
          <div className="flex flex-col xl:flex-row gap-3 items-stretch justify-between">
            <div className="flex flex-wrap gap-2.5 items-center flex-1">
              ...
              >
                ...
                <option value="Istanbul Main">Istanbul Main</option>
                <option value="Munich Transit">Munich Transit</option>
              </select>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative group">
                ...
                <div className="absolute right-0 mt-1.5 hidden group-focus-within:block group-hover:block bg-kp-bg-secondary border border-kp-border rounded-kp-lg shadow-kp-dropdown p-3.5 z-20 w-48 text-xs space-y-2">
                  <span className="font-bold text-kp-text-tertiary block border-b border-kp-border pb-1.5 mb-1.5">Toggle Columns</span>
            ...
            <table className="w-full text-left border-collapse text-theme-sm text-kp-text-secondary">
              <thead>
                <tr className="border-b border-kp-border text-[11px] font-semibold uppercase tracking-wider text-kp-text-tertiary bg-kp-bg-primary/30 select-none">
                  ...
                  {visibleColumns.name && <th onClick={() => handleSort('name')} className="py-3 px-4 cursor-pointer hover:bg-kp-bg-hover transition-colors">Product Details</th>}
                  {visibleColumns.warehouse && <th onClick={() => handleSort('warehouse')} className="py-3 px-4 cursor-pointer hover:bg-kp-bg-hover transition-colors">Warehouse & Location</th>}
                  {visibleColumns.available && <th onClick={() => handleSort('available')} className="py-3 px-4 text-right cursor-pointer hover:bg-kp-bg-hover transition-colors">Available</th>}
                  {visibleColumns.reserved && <th onClick={() => handleSort('reserved')} className="py-3 px-4 text-right cursor-pointer hover:bg-kp-bg-hover transition-colors">Reserved</th>}
                  {visibleColumns.incoming && <th onClick={() => handleSort('incoming')} className="py-3 px-4 text-right cursor-pointer hover:bg-kp-bg-hover transition-colors">Incoming</th>}
                  {visibleColumns.damaged && <th className="py-3 px-4 text-right">Damaged</th>}
                  {visibleColumns.status && <th onClick={() => handleSort('status')} className="py-3 px-4 cursor-pointer hover:bg-kp-bg-hover transition-colors">Status</th>}
                  {visibleColumns.actions && <th className="py-3 px-4 text-center">Actions</th>}
                </tr>
              </thead>
                    ...
                    {visibleColumns.name && (
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-3">
                          <img src={p.image} alt={p.name} className="h-8 w-8 rounded-kp-md object-cover border border-kp-border" />
                          <div>
        ...
        <div className="fixed inset-0 bg-black/40 flex justify-end z-50 animate-fade-in">
          ...
          <div className="w-full max-w-xl bg-kp-bg-secondary shadow-kp-dropdown h-full flex flex-col justify-between overflow-y-auto">
            ...
            <div className="p-6 border-b border-kp-border flex justify-between items-start">
              <div className="flex items-center gap-3">
                <img src={selectedProduct.image} alt={selectedProduct.name} className="h-10 w-10 rounded-kp-md object-cover border border-kp-border" />
                <div>
            ...
            <div className="flex-1 p-6 space-y-6 text-xs text-kp-text-secondary">
              ...
              {drawerSubTab === 'details' && (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <span className="text-[10px] text-kp-text-tertiary uppercase">Barcode</span>
                      <div className="text-kp-text-primary font-semibold mt-0.5">{selectedProduct.barcode}</div>
                    </div>
                    <div>
                      <span className="text-[10px] text-kp-text-tertiary uppercase">Brand</span>
                  ...
                  <div className="border-t border-kp-border pt-4 space-y-2">
                    <h4 className="font-bold text-kp-text-primary uppercase tracking-wider text-[10px]">Stock Config</h4>
                    <div className="grid grid-cols-4 gap-2 text-center text-xs">
                      ...
                      <div className="p-2 rounded bg-kp-bg-primary/50">
                        <span className="text-[9px] text-kp-text-tertiary block uppercase">Reorder</span>
                        <span className="font-bold text-kp-text-primary">{selectedProduct.reorderPoint}</span>
                      </div>
                      <div className="p-2 rounded bg-kp-bg-primary/50">
                        <span className="text-[9px] text-kp-text-tertiary block uppercase">Safety</span>
              ...
              {drawerSubTab === 'locations' && (
                <div className="space-y-4">
                  <div className="p-4 rounded-kp-md border border-kp-border bg-kp-bg-primary/30 flex justify-between items-center">
                    ...
                    <span className="font-bold text-kp-text-primary">{selectedProduct.available} units</span>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-3">
                  {selectedProduct.reserved > 0 ? (
                    <div className="flex justify-between items-center p-3 rounded bg-kp-bg-primary/50 text-xs font-semibold text-kp-text-primary">
                      <span>Shopify Store Sync Lock</span>
                      <span className="text-kp-warning font-bold">{selectedProduct.reserved} units</span>
                    </div>
                  ) : (
                    <p className="text-xs text-kp-text-tertiary italic text-center py-4">No reservations active.</p>
                  )}
                </div>
              )}
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-kp-md bg-kp-accent-muted/10 border border-kp-accent/20 text-kp-text-primary space-y-2">
                    <div className="flex items-center gap-1.5">
                      <SparklesIcon className="h-5 w-5 text-kp-accent animate-pulse" />
                      <h4 className="font-bold text-xs">AI Forecast Score: 94%</h4>
                    </div>
                    <p className="text-[11px] leading-relaxed text-kp-text-secondary">
                      Rotation speed is HIGH. Recommend placing replenishment PO of 250 units on 2026-07-05 to prevent stockout.
                    </p>
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 animate-fade-in">
          <div className="w-full max-w-md bg-kp-bg-secondary rounded-kp-lg p-6 shadow-kp-dropdown border border-kp-border space-y-4 animate-scale-in">
            <div className="flex justify-between items-center border-b border-kp-border pb-3">
              <h3 className="font-bold text-sm text-kp-text-primary">Draft Quick Inbound Shipment</h3>
            ...
            <form
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  alert(`PO Inbound Shipment ${poRef} registered!`);
                } catch (err) {
            ...
            >
              <div>
                <label className="block text-[11px] font-semibold text-kp-text-tertiary uppercase tracking-wider mb-1">Target Warehouse</label>
                <select className="w-full rounded-kp-md border border-kp-border bg-kp-bg-secondary text-kp-text-primary px-3 py-2 focus:outline-none">
                  <option>Istanbul Main Warehouse</option>
                  <option>Munich Transit Terminal</option>
                </select>
              </div>

              <div>
                <label className="block text-[11px] font-semibold text-kp-text-tertiary uppercase tracking-wider mb-1">Reference PO Number</label>
