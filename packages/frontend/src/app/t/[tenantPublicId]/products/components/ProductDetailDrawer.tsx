  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] bg-kp-bg-secondary flex flex-col h-screen w-screen overflow-hidden animate-fade-in">
      ...
      <div className="flex border-b border-kp-border px-8 bg-kp-bg-primary/10 flex-shrink-0">
        {[
          { value: 'details' as DrawerTab, label: 'Detaylar', Icon: InformationCircleIcon },
          { value: 'integrations' as DrawerTab, label: 'Entegrasyonlar', Icon: LinkIcon },
        ...
        <div className="max-w-4xl mx-auto w-full space-y-6">
          ...
          <div className="flex items-center justify-between p-4 bg-kp-accent/5 border border-kp-border rounded-kp-lg flex-shrink-0 mb-4">
            ...
            <div className="text-right">
              <p className="text-[10px] text-kp-text-tertiary">Satış Fiyatı</p>
          ...
          {activeTab === 'details' && (
            <>
              {/* Product info */}
              <div>
                <h4 className="text-[10px] font-bold text-kp-text-tertiary uppercase tracking-widest mb-3">Ürün Bilgileri</h4>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Kategori</p>
                    <p className="font-medium text-kp-text-primary mt-0.5">{category?.name || '—'}</p>
                  </div>
                  <div>
                    <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Barkod</p>
                    <p className="font-mono text-kp-text-secondary mt-0.5">{product.barcode || '—'}</p>
                  </div>
                  <div>
                    <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Muhasebe / ERP Eşleşmesi</p>
                    <p className="font-mono mt-0.5 text-xs">
                      {product.erpCode ? (
                        ...
                        <span className="text-kp-text-tertiary font-normal italic">Eşleşme Yok</span>
                      )}
                    </p>
                  </div>
                  {product.description && (
                    <div className="col-span-2">
                      <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Açıklama</p>
              ...
              <div className="border-t border-kp-border pt-4">
                <h4 className="text-[10px] font-bold text-kp-text-tertiary uppercase tracking-widest mb-3">Envanter & Lojistik</h4>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Stok Adedi</p>
                    <p className="font-medium text-kp-text-primary mt-0.5">{product.stockQuantity} adet</p>
                  </div>
                  <div>
                    <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Maliyet Fiyatı</p>
                  ...
                  <div>
                    <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Taban Fiyat (MSRP)</p>
                    <p className="font-medium text-kp-text-primary mt-0.5">
                      {sym}{parseFloat(product.basePrice.toString()).toLocaleString('tr-TR', { minimumFractionDigits: 2 })}
                    </p>
                  </div>
                  <div>
                    <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Para Birimi</p>
              ...
              {(product.weight || product.width || product.height || product.depth) && (
                <div className="border-t border-kp-border pt-4">
                  <h4 className="text-[10px] font-bold text-kp-text-tertiary uppercase tracking-widest mb-3">Boyutlar & Ağırlık</h4>
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Ağırlık</p>
                      <p className="font-medium text-kp-text-primary mt-0.5">{product.weight ? `${product.weight} kg` : '—'}</p>
                    </div>
                    <div>
                      <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Boyutlar (G×Y×D)</p>
                      <p className="font-medium text-kp-text-primary mt-0.5">
                    ...
                    {desi && (
                      <div className="col-span-2">
                        <p className="text-kp-text-tertiary text-[10px] uppercase tracking-wider">Desi Değeri</p>
              ...
              {product.image && (
                <div className="border-t border-kp-border pt-4">
                  <h4 className="text-[10px] font-bold text-kp-text-tertiary uppercase tracking-widest mb-3">Tüm Görseller</h4>
          ...
          {activeTab === 'integrations' && (
            <div className="space-y-8 divide-y divide-kp-border">
              ...
              <div className="pt-6 space-y-4">
                <h4 className="text-xs font-bold text-kp-text-primary uppercase tracking-wider">Senkronizasyon Geçmişi</h4>
                {logsLoading ? (
                  <div className="flex h-32 items-center justify-center gap-2">
                    <ArrowPathIcon className="h-5 w-5 text-kp-accent animate-spin" />
                    <span className="text-xs text-kp-text-tertiary">Yükleniyor...</span>
                  </div>
                ) : integrationLogs.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center gap-2">
                    <LinkIcon className="h-8 w-8 text-kp-text-tertiary" />
                    <p className="text-sm font-medium text-kp-text-secondary">Entegrasyon kaydı yok</p>
                    <p className="text-xs text-kp-text-tertiary">Bu ürün için henüz entegrasyon işlemi gerçekleşmemiş.</p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-left text-xs">
                      <thead className="bg-kp-bg-primary/40 text-[10px] font-semibold uppercase text-kp-text-tertiary border-b border-kp-border">
                        <tr>
                          ...
                          <th className="py-2 px-3">Sağlayıcı</th>
